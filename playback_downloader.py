...
class DeviceScraper:
    ...
    async def load_local_storage(self) -> bool:
        ...
        if self.storage_path.exists():
            ...
            await self.page.evaluate(f"""(storage) => {{
    ...
    async def _handle_download(self, download):
        ...
        try:
            ...
            self.log(f"[+] Download completed: {filename} (Total: {len(self.completed_downloads)})")
            
            # Organize file immediately (real-time)
            await self._organize_single_file(filename)
            
        except Exception as e:
            self.log(f"[-] Error saving download {filename if 'filename' in locals() else 'unknown'}: {str(e)}")
            import traceback
    ...
    async def _organize_single_file(self, filename: str):
        ...
        try:
            ...
            if old_path.exists():
                ...
                self.log(f"[✓] Organized: {filename} → channel{channel_num}/{new_filename}")
            else:
    ...
    async def login(self, username: str = "scrapper", password: str = "sc@10001") -> bool:
        """Login to device"""
        try:
    ...
    async def get_pagination_info(self) -> Dict:
        ...
        try:
            ...
            self.log(f"[*] Pagination: Page {page_info['current']} of {page_info['total']}")
            return page_info
        except Exception as error:
            ...
            while asyncio.get_event_loop().time() - start_time < timeout_sec:
                ...
                if status['alertVisible'] and status['alertText']:
                    ...
                    if success_match and failure_match:
                        ...
                        self.log(f"[+] Download initiated: {success} success, {failure} failure")
                        ...
                        for wait_count in range(60):  # Wait up to 60 seconds
                            current_downloads = len(self.completed_downloads) - initial_download_count
                            
                            if current_downloads >= expected_files and len(self.pending_downloads) == 0:
                                self.log(f"[+] All {current_downloads} files downloaded successfully")
                                return result
                            
                            if wait_count % 5 == 0:  # Log every 5 seconds
                                self.log(f"[*] Downloaded {current_downloads}/{expected_files} files, {len(self.pending_downloads)} pending")
                            
                            await asyncio.sleep(1)
                        
                        self.log(f"[+] Download completed with {len(self.completed_downloads) - initial_download_count} files")
                        return result
                ...
                if status['infoText']:
                    ...
                    if match:
                        ...
                        if current == total and total > 0:
                            self.log(f"[+] All files queued for download ({current}/{total})")
                            ...
                            for _ in range(60):
                                ...
                                if alert_showing:
                                    ...
                                    # Parse result
                                    success_match = re.search(r'Success (\d+)', result_text)
                                    failure_match = re.search(r'Failure (\d+)', result_text)
                                    if success_match and failure_match:
                            ...
                            self.log(f"[+] Download finished with {len(self.completed_downloads) - initial_download_count} files")
                            return result
                    
                    # Check for stalled progress
                    time_since_change = asyncio.get_event_loop().time() - last_progress_time
                    if no_change_count > 30 and time_since_change > 60 and not status['stopBtnExists']:
...
async def download_playback(scraper: DeviceScraper):
    ...
    try:
        ...
        scraper.log(f"[+] Active channels: {len(active_channels)} (filtered from {len(channels)})")
        ...
        for channel in active_channels:
            ...
            if not await scraper.set_date_range(start_date, end_date):
                scraper.log(f"[-] Failed to set date range untuk channel {channel['label']}")
                continue
            
            if not await scraper.query_playback():
                scraper.log(f"[-] Failed to query playback untuk channel {channel['label']}")
                continue
            ...
            for page in range(1, page_info['total'] + 1):
                if page > 1:
                    ...
                    await scraper.page.evaluate(f"""(targetPage) => {{
                ...
                if table_data:
                    ...
                    if page == 1:
                        ...
                        for idx, file in enumerate(table_data[:3]):
                            scraper.log(f"    {idx + 1}. {file['startTime']} -> {file['endTime']} ({file['type']})")
                else:
                    scraper.log(f"[*] No files found on page {page} of {page_info['total']}")
                    continue
                ...
                while retry_count < max_retries:
                    ...
                    if not download_result['completed']:
                        scraper.log(f"[!] Download halaman {page} tidak selesai dalam timeout")
                        break
                    
                    # Check if there are failures
                    if download_result['failure'] > 0:
                        scraper.log(f"[!] Page {page} has {download_result['failure']} failed files")
                        retry_count += 1
                        ...
                        if retry_count < max_retries:
                            ...
                            scraper.log(f"[!] Max retries reached, skipping remaining failures")
                            break
                    else:
                        # All files succeeded
                        scraper.log(f"[✓] Page {page} completed successfully - all {download_result['success']} files downloaded")
                        break
